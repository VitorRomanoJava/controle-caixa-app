...
function Saidas({
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        setErrorMessage(
        error instanceof Error ? error.message : "Erro ao carregar saídas."
        );
        setIsLoading(false);
      }
    };
  ...
  const handleDelete = async (transaction: Transaction) => {
    const confirmed = window.confirm(
      "Deseja excluir esta saida? Essa acao nao pode ser desfeita."
    );
    if (!confirmed) {
      return;
    }
    ...
    try {
      ...
      onToast("Saída excluída.", "success");
      onRefresh();
    } catch (error) {
      onToast(
        error instanceof Error ? error.message : "Erro ao excluir saída.",
        "error"
      );
    }
  };
    ...
    <div className="space-y-6 pt-6">
      <div>
        <div className="flex items-center justify-between gap-4">
          <h2 className="text-2xl font-semibold text-slate-900">Saídas</h2>
          <VisibilityToggle visible={showValues} onToggle={onToggleValues} />
        </div>
        <p className="mt-2 text-sm text-slate-500">
          Busque, filtre e edite as saídas do mês selecionado.
        </p>
      </div>
